 ...
 *	Whiteboard
 *
 *  Copyright © 2022-2024 Ingenuity i/o. All rights reserved.
 *
...
class Whiteboard : public QObject
{
    ...
    I2_QT5_QML_PROPERTY_WITH_CUSTOM_SETTER(int, windowWidth)
    I2_QT5_QML_PROPERTY_WITH_CUSTOM_SETTER(int, windowHeight)

    I2_QT5_QML_PROPERTY_WITH_CUSTOM_SETTER(int, whiteboardWidth)
    I2_QT5_QML_PROPERTY_WITH_CUSTOM_SETTER(int, whiteboardHeight)

    I2_QT5_QML_PROPERTY(QString, title)
    I2_QT5_QML_PROPERTY(QColor, backgroundColor)
    I2_QT5_QML_PROPERTY(bool, labelsVisible)
    I2_QT5_QML_PROPERTY(bool, canInteractWithElements)
...
public:
    ...
    Q_INVOKABLE void sendClick(const QPointF& position);
...
Q_SIGNALS:
    ...
    void getWindowSize(const QString& agentUUID, const QString& token);
    void getWhiteboardSize(const QString& agentUUID, const QString& token);

    void chatMessage(const QString& text);
    void chatBot(const QString& agentName, const QString& text);
    void chatAs(const QString& name, const QString& text);
...
private Q_SLOTS:
    void _setTitle(const QString& value);
    void _setBackgroundColor(const QColor& value);
    void _getWindowSize(const QString& agentUUID, const QString& token);
    void _getWhiteboardSize(const QString& agentUUID, const QString& token);
    void _chatMessage(const QString& text);
    void _chatBot(const QString& agentName, const QString& text);
    void _chatAs(const QString& name, const QString& text);
    void _clear();
    void _showLabels();
    void _hideLabels();
    void _lockElements();
    void _unlockElements();
...
private:
    ...
    QString _convertByteArrayToImageSource(const QByteArray& rawData);
