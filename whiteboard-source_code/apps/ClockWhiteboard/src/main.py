...
from ClockWhiteboard import *

port = 5670
agent_name = "ClockWhiteboard"
device = None
verbose = False
...
def on_agent_event_callback(event, uuid, name, event_data, my_data):
    try:
        ...
        assert isinstance(agent_object, ClockWhiteboard)
...
# inputs
def Time_Iso_input_callback(io_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, ClockWhiteboard)
        agent_object.Time_IsoI = value
        # add code here if needed
    except:
        print(traceback.format_exc())

def Timestamp_input_callback(io_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, ClockWhiteboard)
        agent_object.TimestampI = value
        # add code here if needed
    except:
        print(traceback.format_exc())
...
def Elementcreated_callback(sender_agent_name, sender_agent_uuid, service_name, tuple_args, token, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, ClockWhiteboard)
        element_id = tuple_args[0]
        agent_object.Elementcreated(element_id, token)
    except:
        print(traceback.format_exc())
...
def actionResult_callback(sender_agent_name, sender_agent_uuid, service_name, tuple_args, token, my_data):
    try:
        ...
        actionSucceeded = tuple_args[0]
        if not actionSucceeded:
            igs.error("Error trying to update text on the Whiteboard")
            igs.service_call("Whiteboard", "clear", None, None)
            agent_object.clean_ids()
    except:
        print(traceback.format_exc())
...
if __name__ == "__main__":
    ...
    igs.definition_set_class("ClockWhiteboard")
    ...
    agent = ClockWhiteboard()

    igs.observe_agent_events(on_agent_event_callback, agent)

    igs.input_create("time_iso", igs.STRING_T, None)
    igs.observe_input("time_iso", Time_Iso_input_callback, agent)
    igs.input_create("timestamp", igs.INTEGER_T, None)
    igs.observe_input("timestamp", Timestamp_input_callback, agent)

    igs.output_create("time_iso", igs.STRING_T, None)
    igs.output_create("timestamp", igs.INTEGER_T, None)
    ...
    timer_id = igs.timer_start(1000, 0, timer_timeout, agent)
