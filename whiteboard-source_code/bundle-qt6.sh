...
echo "Remove system libs that could clash with the OS ones"
rm \
    $SCRIPT_DIR/DistributionKit/lib/ld-linux-x86-64.so.* \
    $SCRIPT_DIR/DistributionKit/lib/libc.so.* \
    $SCRIPT_DIR/DistributionKit/lib/libfontconfig.so.* \
    $SCRIPT_DIR/DistributionKit/lib/libfreetype.so.* \
    $SCRIPT_DIR/DistributionKit/lib/libgcc_s.so.* \
    $SCRIPT_DIR/DistributionKit/lib/libglib-2.0.so.* \
    $SCRIPT_DIR/DistributionKit/lib/libgtk-3.so.* \
    $SCRIPT_DIR/DistributionKit/lib/libpthread.so.* \
    $SCRIPT_DIR/DistributionKit/lib/libdl.so.* \
    $SCRIPT_DIR/DistributionKit/lib/libm.so.* \
    $SCRIPT_DIR/DistributionKit/lib/librt.so.* \
    $SCRIPT_DIR/DistributionKit/lib/libwayland-client.so.* \
    $SCRIPT_DIR/DistributionKit/lib/libwayland-cursor.so.* \
    $SCRIPT_DIR/DistributionKit/lib/libwayland-egl.so.*


echo "Copy libicu* explicitly because cqtdeployer does not do it"
...
cp $ARTIFACTS_NET_SHARE/linux64/binaries/ingescape/lib/libingescape.so.4 $PACKAGE_DIR/opt/$APP_INSTALL_DIR/lib
